    ...
    initComponent: function() {
        Ext.apply(this, {
            ...
            tpl: new Ext.XTemplate(
                '<div class="tweets">',
                    '<tpl for=".">',
                        '<div class="tweet">',
                            '<a href="http://twitter.com/{[values.get("from_user")]}" target="_blank"><img src="{[values.get("profile_image_url")]}"/></a>',
                            '<div class="text"><span class="username"><a href="http://twitter.com/{[values.get("from_user")]}" target="_blank">{[values.get("from_user")]}</a></span> - ',
                            '<span class="date">{[values.get("date_formatted")]}</span><br>{[this.linkifyText(values.get("text"))]}</div>',
                        '</div>',
                    '</tpl>',
                '</div>',
                {
        ...
        Ext.regModel('Tweet', {
            fields: [
                ...
                {name: "date_formatted", convert: function(value, record) { return Ext.Date.format(record.get("created_at_date"), "D M j, g:ia"); }},
            ],
        });
